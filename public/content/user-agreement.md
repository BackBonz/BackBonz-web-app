> **⚠ SAMPLE CONTENT — Replace before launch.** This is a placeholder User Agreement for development and review purposes only. Consult a qualified attorney before publishing.

# User Agreement

*Last Updated: May 1, 2026*

---

## 1. Introduction and Acceptance

Welcome to BackBonz ("we," "us," or "our"). By creating an account or using the BackBonz application and website (the "Service"), you agree to be bound by this User Agreement ("Agreement").

**If you are under 18**, your parent or guardian must review and accept this Agreement on your behalf. We require verifiable parental consent before children under 13 may use the Service.

If you do not agree to these terms, please do not use BackBonz.

---

## 2. Eligibility

BackBonz is designed for users aged **8–16** who have been prescribed a scoliosis brace and their parents/guardians.

- Users aged **13–16** may create accounts with parent/guardian permission
- Users **under 13** require verified parental consent before account creation
- Users **18 and older** may use the Service as parents, guardians, or clinicians

---

## 3. Parental Consent and Supervision

### For Children Under 13

We require **verifiable parental consent** before collecting any personal information from children under 13, in accordance with COPPA.

The consenting parent or guardian:
- Confirms they are the legal parent or guardian of the child
- Agrees to this Agreement and our Privacy Policy on the child's behalf
- Acknowledges they are responsible for supervising their child's use of the Service
- May review, modify, or request deletion of their child's data at any time

### Consent Process

1. Parent/guardian provides their email during account setup
2. A verification email is sent with a confirmation link
3. Account is activated only after the parent/guardian confirms

---

## 4. About BackBonz

BackBonz is a **motivational wellness application** designed to help teens track brace-wear time, build healthy habits, and stay motivated through companion-based engagement.

### BackBonz Does NOT:
- Provide medical advice, diagnosis, or treatment
- Replace the guidance of your orthotist, orthopedic surgeon, or other healthcare professionals
- Guarantee any medical outcome
- Store or transmit health records to third parties

### BackBonz Is NOT a Medical Device

The information and features provided in BackBonz are for **motivational and personal tracking purposes only**. Always follow the recommendations of your healthcare team.

---

## 5. User Accounts

### Account Creation
- You must provide accurate information during registration
- You are responsible for maintaining the confidentiality of your account credentials
- You must notify us immediately of any unauthorized account access

### Account Security
- Use a strong, unique password
- Do not share your account credentials
- Log out of shared devices

---

## 6. Acceptable Use

You agree not to:

- Use the Service for any unlawful purpose
- Attempt to gain unauthorized access to any part of the Service
- Interfere with or disrupt the Service or its servers
- Upload harmful, offensive, or inappropriate content
- Impersonate another person
- Use automated tools to access or scrape the Service

---

## 7. User-Generated Content

BackBonz may allow you to submit journal entries and notes. You retain ownership of content you create. By submitting content, you grant us a limited license to store and display it solely for providing the Service.

You are responsible for all content you submit. Do not include:
- Personal health information beyond what's needed for the Service
- Contact information of others without their consent
- Harmful or distressing content

---

## 8. Account Deletion

You may delete your account at any time through:

1. **In-app**: Settings → Account → Delete Account
2. **Email request**: Send "Delete My Account" to [support.backbonz@gmail.com](mailto:support.backbonz@gmail.com)

Upon account deletion:
- All personal data will be deleted within **30 days**
- Deletion is **permanent and irreversible** — we cannot recover your data after deletion
- Backups are purged within **90 days**

For accounts belonging to children under 13, a parent or guardian must make the deletion request.

---

## 9. Data Export

You have the right to request a copy of your personal data ("data portability").

To request a data export:
1. Email [support.backbonz@gmail.com](mailto:support.backbonz@gmail.com) with the subject "Data Export Request"
2. Include the email address associated with your account
3. We will provide your data in a common machine-readable format within **30 days**

The export will include: account information, brace session logs, journal entries, and activity history.

---

## 10. Intellectual Property

BackBonz and all associated content — including the app design, graphics, fish companion characters, logos, and written content — are owned by BackBonz and protected by copyright and intellectual property laws.

You may not:
- Copy, modify, or distribute our content without permission
- Reverse engineer the application
- Use our trademarks without written permission

---

## 11. Health Disclaimer

BackBonz is a motivational tool, not a medical device.

**We strongly recommend:**
- Following your orthotist's prescribed daily wear-time goals
- Attending all scheduled medical appointments
- Consulting your healthcare team about any concerns with your brace treatment

The BackBonz brace timer is a self-reported tracking tool. We make no guarantee of accuracy. BackBonz session data is not a substitute for clinical assessment.

---

## 12. Limitation of Liability

To the fullest extent permitted by law:

- BackBonz is provided "as is" without warranties of any kind
- We are not liable for any indirect, incidental, or consequential damages
- Our total liability to you shall not exceed the amount you paid for the Service in the past 12 months

---

## 13. Changes to This Agreement

We may update this Agreement from time to time. We will notify you of material changes by:

- Sending a notice to the parent/guardian email on file
- Displaying a notice in the app

Continued use of the Service after notice constitutes acceptance.

---

## 14. Governing Law

This Agreement is governed by the laws of the applicable jurisdiction. Any disputes will be resolved through binding arbitration, except for injunctive relief claims.

---

## 15. Contact Us

Questions about this Agreement? Contact us:

**BackBonz**
Email: [support.backbonz@gmail.com](mailto:support.backbonz@gmail.com)

For account deletion, data export, or COPPA requests, please include the relevant request type in your email subject line.
